...
resliceMM = 1.8; %resolution for reslicing data
...
[meanname, prefix] = mocoFMSub(prefix, fmriname, phase, magn, te1, te2, frmi_readout);
...
%5.) blur data
prefix = smoothSub(3.6, prefix, fmriname); %smooth images
...
function [meanname, prefix] = mocoFMSub(prefix, fmriname, phase, magn, te1, te2, frmi_readout) %motion correct with field map
if isempty(phase) || isempty(magn)
    meanname = mocoSub(prefix, fmriname); %motion correct
    return;
end;
FieldMapSub(fmriname, phase, magn, te1, te2, frmi_readout);
...
function FieldMapSub(fmriname, phase, magnitude, te1, te2, frmi_readout)
