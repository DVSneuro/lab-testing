...
% created by Chris Rorden, and then adapted by David Smith (smith@psychology.rutgers.edu) 
...
%0.) set origin
setOriginSub(strvcat(t1name, fmriname, phase, magn), 1);  %align images to MNI space
...
[meanname, prefix] = mocoFMSub(prefix, fmriname, phase, magn, te1, te2, frmi_readout, blip_dir);
...
%5.) blur data -- probably better to do this with FSL because of SUSAN
prefix = smoothSub(0, prefix, fmriname); %smooth images
...
function [meanname, prefix] = mocoFMSub(prefix, fmriname, phase, magn, te1, te2, frmi_readout, blip_dir) %motion correct with field map
if isempty(phase) || isempty(magn)
    meanname = mocoSub(prefix, fmriname); %motion correct
    return;
end;
FieldMapSub(fmriname, phase, magn, te1, te2, frmi_readout, blip_dir);
...
function FieldMapSub(fmriname, phase, magnitude, te1, te2, frmi_readout, blip_dir)
...
% te1 = 5.19; %short echo time for fieldmap, ms
% te2 = 7.65; %long echo time for fieldmap, ms
% frmi_readout = 0.69 * 64; %EPI readout time for fMRI scan
...
if ~exist('modality','var') %no files specified
 modality = 1;
 fprintf('%s Modality not specified, assuming wrenderanatwrenderanat\n', mfilename);
end
coivox = ones(4,1);
%extract filename 
